...
### 修改计划
- 修改目标：修复 R2 P1×4（D5数据×2、D4结构重叠、D6金句间距）
- 修改级别：全部为词/句级别（四级递进第一级）
- 预计修改幅度：< 3%

### 预期效果标注
- 修改目标维度：D5、D4、D6
- 当前分数：D5=12, D4=7, D6=4
- 预期提升至：D5=13, D4=8, D6=5
- 预期效果说明：消除2处数据偏差可修复D5的R1标注问题；删除重复段落可消除D4结构冗余；拉开金句间距可增强各自冲击力

### 修改明细

| # | 位置 | 原文 | 修改后 | 类型 | 对应问题 |
|---|------|------|--------|------|----------|
| 1 | L37 | 禁用词有 13 个 | 禁用词有 14 个 | 改词 | D5 FE-02 |
| 2 | L109 | 口语化程度从之前的水平提升了 6 分 | 口语化词组从 0 个增加到了 6 个 | 改句 | D5 FE-01 |
| 3 | L75-77 | 两句金句连续出现（间隔2行） | 第二句金句移至L77段内（间隔约5行） | 调整位置 | D6 |
| 4 | L155-157 | 重复结尾段落（"W1写，W2改，R1复查"第二次出现） | 删除 | 删句 | D4 |

### 卡兹克风格自检
- 禁用词扫描：修改部分无新禁用词引入
- 标点禁令：修改部分无冒号/破折号/双引号引入
- 口语化检查：修改后口语化程度未退步
- 连贯性检查：金句移动后与前后文衔接自然；结尾删除后收束更紧凑

### 预期效果验证
- 预期提升：D5 +1, D4 +1, D6 +1（共 +3 分）
- 修改幅度：约 2%（4处词/句级别修改，删除3行）
- 是否达标：待 R1 复检 + R3 验证
